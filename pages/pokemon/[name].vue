        ...
        <template v-else-if="status === 'success' && pokemonData.ok">
            <section
                ...
                <div class="flex w-full items-center justify-center lg:w-auto lg:pl-6">
                    <img :src="pokemonData.response.image" :alt="pokemonData.response.name" width="96" height="96"
                        loading="lazy" class="" />
                    <span class="text-2xl font-semibold">{{ nameTitleCase }}</span>
                </div>
                <div class="flex flex-wrap justify-center gap-8 lg:ml-auto lg:items-center lg:gap-16 lg:pr-10">
                    <div>
