        ...
        <Tabs :default-value="versions[0]" class="mb-8">
            <div class="relative rounded-md shadow-md">
                <TabsList class="inline-flex w-full max-w-full overflow-x-auto">
                    <TabsTrigger v-for="version in versions" :value="version">{{ version }}</TabsTrigger>
                </TabsList>
            </div>
            <TabsContent v-for="(locations, version) in data?.versions" :value="version" :key="version"
                ...
                <div class="location-wrapper gap-6 pt-6">
                    <div v-for="location of locations"
                        ...
                        <div class="mt-2">
                            ...
                            <template v-else>
                                <Badge class="my-2 mr-2">Level {{ location.min_level }} - {{ location.max_level }}
                                </Badge>
                            </template>
